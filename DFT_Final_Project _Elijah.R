...
# 3d: Remove the extra "X" letter from the date column to better join with the other datasets. 
...
# First, we'll merge  as an inner Join "total_no_of_cases_W2" with "covid19_homeless_impact" by 'date' & 'county'
...
ggplot(covid19_homeless_impact_WR2, aes(sample = rooms)) + geom_qq()                                  # rooms_plot
ggplot(covid19_homeless_impact_WR2, aes(sample = rooms_occupied)) + geom_qq()                         # rooms_occupied_plot
ggplot(covid19_homeless_impact_WR2, aes(sample = cumulative_population_cases)) + geom_qq()            # cumulative_population_cases_plot
ggplot(covid19_homeless_impact_WR2, aes(sample = cumulative_homeless_cases)) + geom_qq()              # cumulative_homeless_cases_plot
ggplot(covid19_homeless_impact_WR2, aes(sample = total_population_by_percentage)) + geom_qq()         # total_population_by_percent_plot
ggplot(covid19_homeless_impact_WR2, aes(sample = total_homless_population_by_percentage)) + geom_qq() # total_homeless_pop_by_percent_plot
